...
<!DOCTYPE html>
    ...
    <body class="bg-gray-100 min-h-screen">
                                ...
                                <tbody id="orderTableBody" class="text-sm">
                                    <c:forEach var="o" items="${ordersList}" varStatus="loop">
                                        <c:set var="statusColor">
                                            <c:choose>
                                                <c:when test="${o.status == 'Processing'}">bg-orange-100 text-orange-600</c:when>
                                                <c:when test="${o.status == 'Shipped'}">bg-blue-100 text-blue-600</c:when>
                                                <c:when test="${o.status == 'Delivered'}">bg-green-100 text-green-600</c:when>
                                        ...
                                        <tr class="order-row border-b border-gray-50 hover:bg-gray-50/50 transition"
                                            ...
                                            <td class="px-6 py-4 font-semibold text-gray-700">${o.customer != null ? o.customer.shopName : 'Unknown'}</td>
                                            <td class="px-6 py-4 text-gray-500 text-xs">
                                                <fmt:formatDate value="${o.orderDate}" pattern="dd-MM-yyyy" />
                                            </td>
                                            <td class="px-6 py-4 font-bold text-blue-600 text-xs">${o.deliveryBoy != null ? o.deliveryBoy.fullName : 'Not Assigned'}</td>
                                            <td class="px-6 py-4 text-gray-500 text-xs truncate max-w-[150px]" title="${o.itemsSummary}">${o.itemsSummary}</td>
                                            <td class="px-6 py-4 font-black text-emerald-600">&#x20B9;${o.totalAmount}</td>
                                            <td class="px-6 py-4">
                                                <span class="px-3 py-1 rounded-full text-xs font-bold ${statusColor}">${o.status}</span>
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                <button onclick="openOrderModal(this.closest('tr'))" class="text-gray-400 hover:text-emerald-600 transition p-2 bg-white rounded-full shadow-sm border border-gray-100" title="View Details">👁️</button>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
        ...
        <div id="orderViewModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm px-4">
            <div class="bg-white rounded-2xl shadow-2xl w-[600px] overflow-hidden transform transition-all">
                <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-emerald-50">
                    ...
                    <button onclick="closeOrderModal()" class="text-gray-400 hover:text-red-500 text-2xl leading-none">&times;</button>
                </div>
                <div class="p-6">
