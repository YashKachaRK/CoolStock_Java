...
<div class="flex">
	...
	<div class="ml-64 p-8 w-full">
		...
		<!-- ── Header ──────────────────────────────────────────────────── -->
		<div class="bg-gradient-to-r from-slate-800 to-gray-700 text-white p-7 rounded-2xl mb-8 flex justify-between items-center shadow-lg">
			<div>
		...
		<div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
			...
			<div class="flex justify-between items-center p-6 border-b bg-orange-50">
				<div>
					...
					<p class="text-gray-400 text-sm mt-0.5">Review applicants from the website's Apply form</p>
				</div>
			...
			<div class="flex gap-2 px-6 pt-5 pb-1">
				<button onclick="filterCards('ALL')"      id="tab-ALL"      class="tab-btn active-tab px-4 py-1.5 rounded-full text-sm font-semibold transition">All</button>
				<button onclick="filterCards('PENDING')"  id="tab-PENDING"  class="tab-btn px-4 py-1.5 rounded-full text-sm font-semibold transition">⏳ Pending</button>
				<button onclick="filterCards('ACCEPTED')" id="tab-ACCEPTED" class="tab-btn px-4 py-1.5 rounded-full text-sm font-semibold transition">✅ Accepted</button>
				<button onclick="filterCards('REJECTED')" id="tab-REJECTED" class="tab-btn px-4 py-1.5 rounded-full text-sm font-semibold transition">❌ Rejected</button>
			</div>

			<style>
				.tab-btn      { background:#f3f4f6; color:#4b5563; }
				.active-tab   { background:#f97316; color:#ffffff; }
			</style>
				...
				<c:choose>
					<c:when test="${empty applications}">
						...
						<div class="py-16 text-center text-gray-400">
							...
							<p class="text-sm mt-1">Applications submitted from the website will appear here.</p>
						</div>
					</c:when>
					<c:otherwise>
						<c:forEach var="app" items="${applications}">
							...
							<c:choose>
								<c:when test="${app.status == 'ACCEPTED'}">
									<c:set var="statusBadge" value="bg-green-100 text-green-700"/>
									<c:set var="statusLabel" value="✅ Accepted"/>
								</c:when>
								<c:when test="${app.status == 'REJECTED'}">
									<c:set var="statusBadge" value="bg-red-100 text-red-600"/>
									<c:set var="statusLabel" value="❌ Rejected"/>
								</c:when>
								<c:otherwise>
									<c:set var="statusBadge" value="bg-orange-100 text-orange-600"/>
							...
							<div class="app-card border-2 border-gray-100 rounded-2xl p-5 hover:border-orange-200 transition fade-in"
								...
								<div class="flex justify-between items-start">
									...
									<div class="flex items-center gap-4">
										<div class="w-12 h-12 ${avatarBg} rounded-2xl flex items-center justify-center text-2xl flex-shrink-0">
											${roleEmoji}
										</div>
										<div>
											<p class="font-bold text-gray-800 text-lg">${app.fullName}</p>
											<p class="text-gray-500 text-sm">
												📞 ${app.phone} &nbsp;|&nbsp; 📧 ${app.email}
											</p>
											<p class="text-gray-400 text-xs mt-1">
												Applied for:
												<span class="font-semibold ${roleColor}">${app.role}</span>
												&nbsp;|&nbsp;
												<fmt:formatDate value="${app.appliedAt}" pattern="dd MMM yyyy" type="date"/>
												<c:if test="${app.appliedAt == null}">—</c:if>
												&nbsp;|&nbsp;
												<span class="font-semibold px-2 py-0.5 rounded-full text-xs ${statusBadge}">${statusLabel}</span>
											</p>
										</div>
									</div>
									...
									<c:if test="${app.status == 'PENDING'}">
										<div class="flex gap-3 flex-shrink-0">
											...
											<!-- Approve form -->
											<form action="${pageContext.request.contextPath}/admin/applications/approve"
											...
											<!-- Reject form -->
											<form action="${pageContext.request.contextPath}/admin/applications/reject"
											      method="post" style="display:inline;">
												<input type="hidden" name="id" value="${app.id}">
												<button type="submit"
				...
				<tbody class="text-gray-700">
					<c:forEach var="staff" items="${activeStaff}">
						<tr class="border-b hover:bg-gray-50">
							...
							<td class="px-6">
								<c:choose>
									<c:when test="${staff.role == 'Manager'}">📊 Manager</c:when>
									<c:when test="${staff.role == 'Delivery'}">🛵 Delivery</c:when>
					...
					<c:if test="${empty activeStaff}">
						<tr>
							<td colspan="4" class="py-4 text-center text-gray-400">No active staff found.</td>
						</tr>
					</c:if>
				</tbody>
			</table>
...
<c:if test="${not empty toastMsg}">
	<div id="toast"
	     class="fixed bottom-6 right-6 text-white px-6 py-3 rounded-2xl shadow-2xl font-semibold z-50
